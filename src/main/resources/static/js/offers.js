        ...
        .then(data => {
            ...
            document.querySelectorAll('.delete-btn').forEach(button => {
                button.addEventListener('click', (e) => {
                    ...
                    fetch(`/api/offers/delete/${assetId}`, {
                        method: 'POST'
                    })
                        .then(async response => {
                            if (!response.ok) {
            ...
            let isCompetitionCan = true

            document.getElementById('competition-btn').addEventListener('click', () => {
                if(isCompetitionCan) {
                    isCompetitionCan = false
                    const competitionBtn = document.getElementById('competition-btn')
                    if (competitionBtn.dataset.active === 'true') {
                        alert("Обробляю запит...")

                        competitionBtn.innerHTML = falseSvg
                        competitionBtn.dataset.active = 'false'
                    } else {
                        alert("Обробляю запит...")

                        competitionBtn.innerHTML = trueSvg
                        competitionBtn.dataset.active = 'true'
                    }


                    fetch('/api/config/change-offer-competition', {
                        method: 'GET'
                    }).then(response => {
                        if (!response.ok) {
